...
def griddata_regular(x, y, z, xvals, yvals):
    ...
    znew = num.empty(nx*ny, dtype=num.float64)
    znew[:] = num.nan
    znew[zindi] = z
...
class GMT(object):
    ...
    def draw_layout(self, layout):
        ...
        rects = num.array(layout.get_sizes(), dtype=num.float64)
        rects_wid = rects[:, 0, 0]
        rects_hei = rects[:, 0, 1]
        ...
        # points = num.array(corners, dtype=num.float64)
...
if __name__ == '__main__':
    ...
    gmt = GMT()
    gmt.pscoast(R='-30/330/-75/75', J='m0.03c', B='a30g30', G=(0, 128, 0),
                S=(255, 255, 255), A='5000', W='thinnest')
    gmt.save(pjoin(examples_dir, 'example0.pdf'))
